...
export class Session {
   ...
   */
  private handleInputMessage(message: Message & { type: MessageType.Input }): void {
   ...
   */
  private handleHashMessage(message: Message & { type: MessageType.Hash }): void {
  ...
  private handleSyncMessage(
    message: (Message & { type: MessageType.Sync }) | (Message & { type: MessageType.StateSync })
  ): void {
   ...
   */
  private handleSyncRequest(message: Message & { type: MessageType.SyncRequest }): void {
   ...
   */
  private handleJoinRequest(peerId: string, message: Message & { type: MessageType.JoinRequest }): void {
    if (!this._isHost) return;
   ...
   * Handle join accept.
   */
  private handleJoinAccept(message: Message & { type: MessageType.JoinAccept }): void {
    this.setState("lobby");
   ...
   * Handle join reject.
   */
  private handleJoinReject(message: Message & { type: MessageType.JoinReject }): void {
    this.setState("disconnected");
   ...
   */
  private handlePlayerJoined(message: Message & { type: MessageType.PlayerJoined }): void {
    const playerInfo: PlayerInfo = {
      id: message.playerId,
      connectionState: "connected",
   ...
   */
  private handlePlayerLeft(message: Message & { type: MessageType.PlayerLeft }): void {
   ...
   */
  private sendToPeer(peerId: string, message: Message, reliable: boolean): void {
    const encoded = encodeMessage(message);
  ...
  private sendToHost(message: Message): void {
    ...
    for (const player of this._players.values()) {
      if (player.isHost && player.id !== this.localPlayerId) {
        this.sendToPeer(player.id as string, message, isReliableMessage(message));
        return;
      }
    }
