...
describe("RollbackEngine", () => {
  ...
  describe("multi-player inputs", () => {
    ...
    it("should apply inputs from multiple players", () => {
      ...
      engine.receiveRemoteInput(remotePlayer, asTick(0), new Uint8Array([128, 138])); // +10 y
  ...
  describe("rollback on misprediction", () => {
    ...
    it("should rollback when remote input differs from prediction", () => {
      ...
      engine.receiveRemoteInput(remotePlayer, asTick(1), new Uint8Array([118, 128])); // -10 x
    ...
    it("should report rollback ticks in result", () => {
      ...
      engine.receiveRemoteInput(remotePlayer, asTick(5), new Uint8Array([129, 128]));
    ...
    it("should not rollback when prediction matches", () => {
      ...
      engine.receiveRemoteInput(remotePlayer, asTick(1), new Uint8Array([128, 128]));

      // Tick 2
      engine.setLocalInput(asTick(2), new Uint8Array([128, 128]));
      engine.receiveRemoteInput(remotePlayer, asTick(2), new Uint8Array([128, 128]));
      const result = engine.tick();
  ...
  describe("dynamic player management", () => {
    it("should handle player join mid-game", () => {
      ...
      engine.receiveRemoteInput(newPlayer, asTick(5), new Uint8Array([128, 138])); // +10 y
      engine.tick();
    ...
    it("should handle player leave", () => {
      ...
      for (let i = 0; i < 3; i++) {
        engine.setLocalInput(asTick(i), new Uint8Array([128, 128]));
        engine.receiveRemoteInput(otherPlayer, asTick(i), new Uint8Array([138, 128])); // +10 x
        engine.tick();
      }
  ...
  describe("state sync", () => {
    ...
    it("should restore state from sync", () => {
      ...
      engine2.setState(syncState.tick, syncState.state, syncState.playerTimeline);
  ...
  describe("edge cases", () => {
    ...
    it("should handle rapid rollbacks", () => {
      ...
      for (let i = 1; i <= 5; i++) {
        engine.receiveRemoteInput(remote, asTick(i), new Uint8Array([138, 128]));
      }
