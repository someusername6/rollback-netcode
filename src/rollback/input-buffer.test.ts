...
describe("InputBuffer", () => {
  ...
  describe("isPlayerActive", () => {
    it("should return false for unknown player", () => {
      const buffer = new InputBuffer();
      assert.strictEqual(buffer.isPlayerActive(asPlayerId("unknown"), asTick(0)), false);
    });
  ...
  describe("receiveInput and getInput", () => {
    ...
    it("should ignore input for unknown player", () => {
      const buffer = new InputBuffer();
      buffer.receiveInput(asPlayerId("unknown"), asTick(0), new Uint8Array([1]));
      assert.strictEqual(buffer.getInput(asPlayerId("unknown"), asTick(0)), undefined);
    });
  ...
  describe("usedInputs and misprediction", () => {
    ...
    it("should check hasMisprediction in range", () => {
      ...
      assert.strictEqual(buffer.hasMisprediction(p1, asTick(0), asTick(4)), false);
      assert.strictEqual(buffer.hasMisprediction(p1, asTick(5), asTick(10)), true);
      assert.strictEqual(buffer.hasMisprediction(p1, asTick(6), asTick(10)), false);
    });
  });
  ...
  describe("multiple players", () => {
    it("should handle multiple players independently", () => {
      ...
      assert.deepStrictEqual(buffer.getInput(p1, asTick(0)), new Uint8Array([1]));
      assert.deepStrictEqual(buffer.getInput(p2, asTick(0)), new Uint8Array([2]));
      assert.deepStrictEqual(buffer.getInput(p3, asTick(0)), new Uint8Array([3]));
    });
